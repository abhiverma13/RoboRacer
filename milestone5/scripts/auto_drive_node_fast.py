...
class AutoDrive(Node):
    ...
    def __init__(self):
        ...
        self.closest_point_history_size = 5
        
        # Configuration Parameters
        self.speed = 0.0
        self.speed_history = []
        self.speed_history_size = 10
        self.min_gap_distance = 1.5
        ...
        self.declare_parameter('num_laps', -1) # if num_laps==-1 then run indefinitely
    ...
    def get_speed(self, angle, best_point_index, ranges):
        ...
        """
        if 0 <= abs(np.rad2deg(angle)) <= 1.5:
            speed = 2.6 + math.exp(0.04 * abs(ranges[best_point_index]))
            if self.print_speeds:
                print("speed 1: ", speed)
        elif 1.5 < abs(np.rad2deg(angle)) <= 7:
            speed = 2.5
            if self.print_speeds:
                print("speed 2: ", speed)
        elif 7 < abs(np.rad2deg(angle)) <= 15:
            speed = 2.0
            if self.print_speeds:
                print("speed 3: ", speed)
        else:
    ...
    def preprocess_lidar_scan(self, scan_msg):
        ...
        for i in range(self.truncated_start_index, self.truncated_end_index):
            if np.isnan(scan_msg.ranges[i]):
                ...
                index_size = i - midpoint
                lidar_angle = index_size * scan_msg.angle_increment
                distance_from_center = np.sin(lidar_angle) * scan_msg.ranges[i]
                if abs(distance_from_center) < 0.15 and scan_msg.ranges[i] < 1.5:
                    filtered_ranges.append(0.0)
                else:
    ...
    def pid_control(self, error):
        ...
        max_steering = np.deg2rad(30)  # ±30° limit
        angle = np.clip(angle, -max_steering, max_steering)

        if self.print_angles:
            print("original angle: ", np.rad2deg(angle))

        if np.abs(angle) < np.deg2rad(3):
            angle = np.square(angle) * np.sign(angle) * 0.7

        elif np.abs(angle) < np.deg2rad(5):
            angle = np.square(angle) * np.sign(angle)

        elif np.abs(angle) < np.deg2rad(self.large_angle_threshold):
            angle = angle * 0.55
    ...
    def safety_stop(self, ranges) -> bool:
        ...
        minimum_distance = np.min(ranges[midpoint-15:midpoint+15])
        self.closest_point_history.append(minimum_distance)
        if len(self.closest_point_history) > self.closest_point_history_size:
            self.closest_point_history.pop(0)
        if np.mean(self.closest_point_history) < 0.5:
            self.publish_control2(0.0)
            print("EMERGENCY STOP")
            self.stop = True
            return True
        return False
    
    def new_safety_stop(self, scan_msg, ranges, start_index, end_index, steering_angle) -> bool:
        ...
        if np.mean(self.largest_gap_distance_history) < self.stop_threshold:
            self.publish_control(steering_angle, 0.3)
            print("EMERGENCY STOP GAP DISTANCE")
            self.stop = True
            return True
    ...
    def publish_control2(self, speed):
        new_msg = AckermannDriveStamped()
        new_msg.drive.speed = speed
        self.publisher_.publish(new_msg)
        self.speed = speed
    ...
    def scan_callback(self, scan_msg):
        ...
        # Safety stop
        if self.safety_stop(scan_msg.ranges):
            return
        ...
        # New Safety stop
        if self.new_safety_stop(scan_msg, filtered_ranges, start_index, end_index, steering_angle):
            return
    ...
    def process_image_for_comparison(self, image):
        ...
        # Crop
        cropped_h = int(0.50 * h)  # Keep top 40%
        cropped_w = int(0.50 * w)  # Keep left 80%
