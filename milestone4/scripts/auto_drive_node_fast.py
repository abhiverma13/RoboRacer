...
class AutoDrive(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('num_laps', 5) # if num_laps==-1 then run indefinitely
    ...
    def get_speed(self, angle, best_point_index, ranges):
        ...
        if 0 <= abs(np.rad2deg(angle)) <= 2:
            speed = 2.2 + math.exp(0.04 * abs(ranges[best_point_index]))
            if self.print_speeds:
                print("speed 1: ", speed)
        elif 2 < abs(np.rad2deg(angle)) <= 7:
            speed = 2.2
            if self.print_speeds:
                print("speed 2: ", speed)
